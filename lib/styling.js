/* eslint-disable max-len */

export const spacing = {};
spacing.a1 = 0;
spacing.a2 = 2;
spacing.a3 = 4;
spacing.a4 = 8;
spacing.a5 = 16;
spacing.a6 = 24;
spacing.a7 = 32;
spacing.a8 = 48;
spacing.a9 = 72;
spacing.page = spacing.a7;
spacing.pageA2 = spacing.a5;

export const colors = {};
colors.a1 = '#002b36';
colors.a2 = '#073642';
colors.a3 = '#586e75';
colors.a4 = '#657b83';
colors.a5 = '#839496';
colors.a6 = '#93a1a1';
colors.a7 = '#eee8d5';
colors.a8 = '#fdf6e3';
colors.a9 = '#b58900';
colors.a10 = '#cb4b16';
colors.a11 = '#dc322f';
colors.a12 = '#d33682';
colors.a13 = '#6c71c4';
colors.a14 = '#268bd2';
colors.a15 = '#2aa198';
colors.a16 = '#859900';
colors.white = '#fff';
colors.transparent = 'rgba(0, 0, 0, 0)';
colors.inherit = 'inherit';
colors.text = colors.a5;
colors.border = colors.a2;
colors.background = colors.a1;
colors.nav = colors.a15;
colors.links = colors.a9;
colors.thoughtsBg = colors.a2;
colors.h1 = colors.white;
colors.h2 = colors.white;
colors.h3 = colors.a13;
colors.h4 = colors.a16;

export const bps = {};
bps.a1 = 500;
bps.a2 = 800;
bps.a3 = 1000;
bps.a4 = 1200;

export const fontSizes = {};
fontSizes.a1 = 12;
fontSizes.a2 = 14;
fontSizes.a3 = 16;
fontSizes.a4 = 20;
fontSizes.a5 = 28;

export const lineHeights = {};
lineHeights.normal = '1.4em';
lineHeights.close = '1em';

export const fontFamilies = {};
fontFamilies.normal =
  '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif';

export const fontWeights = {};
fontWeights.thin = '200';
fontWeights.normal = '300';
fontWeights.bold = '600';

export const borderRadii = {};
borderRadii.a1 = 4;

export const zIndexes = {};

export const speeds = {};
speeds.a1 = 0.1;
